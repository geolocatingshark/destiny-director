...
REFERENCE_DATE = dt.datetime(2025, 1, 21, 17, tzinfo=dt.timezone.utc)
...
@lb.command("gunsmith", "The gunsmith's wares")
@lb.implements(lb.SlashCommand)
async def gunsmith_command(ctx: lb.Context):
    navigator = NavigatorView(pages=reset_pages)
    await navigator.send(ctx.interaction)


def register(bot):
    bot.command(gunsmith_command)
    bot.listen()(on_start)
